...
class EDD_Recurring_Gateway {
	...
	public function process_checkout( $purchase_data ) {
		...
		if ( ! wp_verify_nonce( $purchase_data['gateway_nonce'], 'edd-gateway' ) ) {
			wp_die( __( 'Nonce verification has failed', 'edd' ), __( 'Error', 'edd-recurring' ), array( 'response' => 403 ) );
		}
		...
		if ( empty( $this->user_id ) ) {
			$subscriber = new EDD_Customer( $this->email );
		} else {
			$subscriber = new EDD_Customer( $this->user_id, true );
		}
		...
		foreach ( $this->purchase_data['cart_details'] as $key => $item ) {
			...
			if( edd_get_option( 'recurring_one_time_discounts' ) ) {
				$recurring_amount = $item['subtotal'] + $item['tax'];
			} else {
			...
			// Determine tax amount for any fees
			$fee_tax = edd_calculate_tax( $fees );
			...
			$args = apply_filters( 'edd_recurring_subscription_pre_gateway_args', array(
				...
				'recurring_tax'    => edd_sanitize_amount( $item['tax'] ),
	...
	public function record_signup() {
		...
		if ( ! $this->offsite ) {
			...
			edd_update_payment_status( $this->payment_id, 'publish' );

		}

		// Set subscription_payment
		edd_update_payment_meta( $this->payment_id, '_edd_subscription_payment', true );
		...
		foreach ( $this->subscriptions as $subscription ) {
			...
			$args        = array(
				...
				'expiration'        => $subscriber->get_new_expiration( $subscription['id'], $subscription['price_id'] ),
