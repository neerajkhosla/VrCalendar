...
class EDD_Recurring_2Checkout extends EDD_Recurring_Gateway {
	...
	public function complete_signup() {
		...
		foreach( $this->subscriptions as $key => $subscription ) {
			...
			$args['li_' . $i . '_recurrence']  = $subscription['frequency'] . ' ' . ucfirst( $subscription['period'] );
			...
			$args['li_' . $i . '_price']       = $subscription['recurring_amount'];
			$args['li_' . $i . '_product_id']  = $subscription['id'];
			$args['li_' . $i . '_name']        = $subscription['name'];
			$args['li_' . $i . '_quantity']    = '1';
			$args['li_' . $i . '_tangible']    = 'N';
			$args['li_' . $i . '_startup_fee'] = $subscription['initial_amount'] - $subscription['recurring_amount'];
	...
	public function can_cancel( $ret, $subscription ) {
		...
		if( $subscription->gateway === '2checkout' && ! empty( $subscription->profile_id ) && 'active' === $subscription->status ) {
