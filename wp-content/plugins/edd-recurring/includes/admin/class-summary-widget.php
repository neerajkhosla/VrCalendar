	...
	public function get_estimated_revenue( $days = 0 ) {
		...
		if( empty( $days ) ) {
			...
			if( false === $amount ) {
				...
				$amount = $wpdb->get_var( "SELECT sum(recurring_amount) FROM {$wpdb->prefix}edd_subscriptions WHERE expiration >= '$now' AND 1=1 AND status = 'active';" );
			...
			if( false === $amount ) {
				...
				$now    = date( 'Y-n-d H:i:s', strtotime( 'now' ) );
				$amount = $wpdb->get_var( "SELECT sum(recurring_amount) FROM {$wpdb->prefix}edd_subscriptions WHERE expiration >= '$now' AND expiration <= '$date' AND status = 'active';" );
