...
class VRCalendar extends VRCSingleton {
    ...
    function paypalReturn() {
        ...
        $thank_you_page = $usermeta['thank_you_page'];
         /*Custom code*/   
        $txn_id = $_POST['txn_id'];
        ...
        if(trim($thank_you_page)!=""){
            wp_redirect( add_query_arg($data,$thank_you_page));
        }
        else{
    ...
    function paypalPayment() {
        ...
        if($usermeta['payment_mode'] == 'sandbox')
            $paypal_url = 'https://www.sandbox.paypal.com/cgi-bin/webscr';
		
        $paypal_url = $paypal_url.'?'.http_build_query($paypal_vars);
		echo "<script> window.open('".$paypal_url."','_top');</script>";
       // header('location:'.$paypal_url);
        exit;
    }
	/*Custom function for getting author id*/	
	function getUserIdFromBookingId($booking_id){
            global $wpdb;
            $querystr= "select calendar_author_id from ".$wpdb->prefix."vrcalandar_bookings as vrb inner join ".$wpdb->prefix."vrcalandar as vr on vrb.booking_calendar_id = vr.calendar_id where vrb.booking_id=".$booking_id."";
            $pageposts = $wpdb->get_results($querystr, OBJECT);
            return $pageposts[0]->calendar_author_id;
    }
