...
class VRBookingsTable extends WP_List_Table {
		...
		{
			$actions = array(
				'delete' => '<a href="' .admin_url('admin.php?page='.VRCALENDAR_PLUGIN_SLUG.'-dashboard&view=bookings&vrc_cmd=VRCalendarAdmin:deleteBooking&bid='.$booking['booking_id'].'&cal_id='.$_GET['cal_id']). '">'.__('Delete', VRCALENDAR_PLUGIN_TEXT_DOMAIN).'</a>'
			);

			if($booking['booking_admin_approved'] == 'no') {
				$actions['approve_booking'] = '<a href="' .admin_url('admin.php?page='.VRCALENDAR_PLUGIN_SLUG.'-dashboard&view=bookings&vrc_cmd=VRCalendarAdmin:approveBooking&bid='.$booking['booking_id'].'&cal_id='.$_GET['cal_id']). '">'.__('Approve', VRCALENDAR_PLUGIN_TEXT_DOMAIN).'</a>';
			}
		...
		{
			$actions = array(
				'edit' => '<a href="' .admin_url('admin.php?page='.VRCALENDAR_PLUGIN_SLUG.'-dashboard&view=bookings&action=editbooking&bid='.$booking['booking_id'].'&cal_id='.$_GET['cal_id']). '">'.__('Edit', VRCALENDAR_PLUGIN_TEXT_DOMAIN).'</a>',
				'delete' => '<a href="' .admin_url('admin.php?page='.VRCALENDAR_PLUGIN_SLUG.'-dashboard&view=bookings&vrc_cmd=VRCalendarAdmin:deleteBooking&bid='.$booking['booking_id'].'&cal_id='.$_GET['cal_id']). '">'.__('Delete', VRCALENDAR_PLUGIN_TEXT_DOMAIN).'</a>'
			);
...
if($action == 'editbooking'){
	$bid = $_GET['bid'];  
	require(VRCALENDAR_PLUGIN_DIR.'/Admin/Views/EditBooking.php');
}else{
	?>
	<h2>
